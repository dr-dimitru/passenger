...
function install_node_and_modules()
{
	if [[ "$install_node_and_modules" = "" ]]; then
		...
		if [[ -e /host_cache ]]; then
                    if [[ "$TRAVIS_OS_NAME" == 'osx' ]]; then
			if [[ ! -e /host_cache/node-v0.10.20-darwin-x64.tar.gz ]]; then
				run curl --fail -L -o /host_cache/node-v0.10.20-darwin-x64.tar.gz \
					https://nodejs.org/dist/v0.10.20/node-v0.10.20-darwin-x64.tar.gz
			fi
			run tar xzf /host_cache/node-v0.10.20-darwin-x64.tar.gz
                    else
			...
			run tar xzf /host_cache/node-v0.10.20-linux-x64.tar.gz
                    ...
                    if [[ "$TRAVIS_OS_NAME" == 'osx' ]]; then
			run curl --fail -L -O https://nodejs.org/dist/v0.10.20/node-v0.10.20-darwin-x64.tar.gz
			run tar xzf node-v0.10.20-darwin-x64.tar.gz
                    else
			...
			run tar xzf node-v0.10.20-linux-x64.tar.gz
                ...
                if [[ "$TRAVIS_OS_NAME" == 'osx' ]]; then
                    export PATH=`pwd`/node-v0.10.20-darwin-x64/bin:$PATH
                else
